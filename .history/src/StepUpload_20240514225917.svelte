<script lang="ts">
  ...
  import BlurOption from "./BlurOption.svelte";
  import RoundedOption from "./RoundedOption.svelte";
  import RotateOption from "./RotateOption.svelte";
  ...
  onMount(() => {
    ...
    dropzone.on("success", (file, response: any) => {
      ...
      //   const imageBlur = cloudinary.image(publicId).effect(effectType);
      //   imageStatus.set(ImageStatus.DONE);
      //   originalImage.set(url);
  ...
  const transformImage = () => {
    ...
    if ($buttonSelected.buttonRounded) {
      console.log(selectedRoundedOption);
      
      let customValueConverted= $selectedRoundedOption === "customRadius" ? $radiusValue.split(",") : $radiusValue
      console.log(customValueConverted);
      
      console.log($radiusValue);
      
      let typeRounded= $selectedRoundedOption === "maximunRadius" ? max() : byRadius($selectedRoundedOption === "singleRadius" ? Number($radiusValue) :  Number(customValueConverted[0]), Number(customValueConverted[1]), Number(customValueConverted[2]), Number(customValueConverted[3]))

      console.log(typeRounded);
      ...
      const imageRounded = cloudinary
        ...
        console.log(imageRounded.toURL());
...
>
  ...
  {#if $imageStatus === ImageStatus.UPLOADING}
    <div
      class="absolute w-full h-full bg-red-50 flex justify-center items-center opacity-50"
    >
      <div role="status">
        <svg
          aria-hidden="true"
