<script lang="ts">
  ...
  const transformImage = () => {
    ...
    if ($buttonSelected.buttonRounded) {
      ...
      let typeRounded= $selectedRoundedOption === "maximunRadius" ? max() : $selectedRoundedOption === "singleRadius" ? byRadius(Number($radiusValue)) 
      : 
      byRadius(Number(customValueConverted[0]), Number(customValueConverted[1]), Number(customValueConverted[2]) , Number(customValueConverted[3]))
