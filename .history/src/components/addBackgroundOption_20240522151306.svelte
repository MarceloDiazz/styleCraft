  ...
  {#if $selectTypeImageBackground === "noTransparent"}
    <div>
      ...
      <div class="flex flex-col gap-5 mt-5">
        ...
        <button
            on:click={()=>{
                selectColorBackground.set("predominant")
            }}
          class={`border rounded-xl p-3  ${$selectColorBackground === "predominant" ? "border-blue-500" : "border-gray-200"}`}
          >Predominante color</button
        >
        <!-- generativeFill -->
        <button
            on:click={()=>{
                selectColorBackground.set("generativeFill")
            }}
          class={`border rounded-xl p-3  ${$selectColorBackground === "generativeFill" ? "border-blue-500" : "border-gray-200"}`}
